...
(function () {
  ...
  // ── Gemini API Key Management (disabled — key is hardcoded above) ────────────
  window.quizSaveGeminiKey = function () { /* key is built-in, no action needed */ };

  // ── Gemini Quiz Generation ────────────────────────────────────────────────────
  async function generateQuizWithGemini(text, numQ) {
    if (!GEMINI_API_KEY) {
      _toast('❌ Set your Gemini API key in the Quiz settings first!');
...
${text.substring(0, 8000)}`;
  ...
  window.loadAdminQuizList = async function () {
    const adminCon = document.getElementById('quiz-admin-list-admin');
    const teacherCon = document.getElementById('quiz-admin-list');
    const container = (adminCon && adminCon.offsetParent !== null) ? adminCon
                    : (teacherCon && teacherCon.offsetParent !== null) ? teacherCon
                    : adminCon || teacherCon;
    if (!container) return;
    ...
    try {
      ...
      let snap;
      try {
        const q = query(collection(window._db, 'quizzes'), orderBy('createdAt', 'desc'));
        snap = await getDocs(q);
      } catch(indexErr) {
        snap = await getDocs(collection(window._db, 'quizzes'));
      }
      const quizzes = snap.docs.map(d => ({ id: d.id, ...d.data() }));

      if (!quizzes.length) {
        container.innerHTML = '<div style="text-align:center;padding:20px;color:var(--muted)">📝 No quizzes yet. Teacher must publish one first.</div>';
        return;
      }
      ...
      container.innerHTML = quizzes.map(qz => `
        <div class="li">
          ...
          <div class="li-info">
            <div class="li-title">${qz.title || 'Untitled'}</div>
      ...
      console.error('[QUIZ] loadAdminQuizList error:', e);
    }
  };
